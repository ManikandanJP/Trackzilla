...
public class TzaController {
    ...
    @RequestMapping(value = "/releases/read", method = RequestMethod.GET)
    public Ticket getReleaseByTicketStatus() {
        System.out.println("Executed here");
        List<Release> releases = releaseService.findByTKT();
        for( Release rel : releases){
            int i = 0;
            System.out.println(rel.getTickets().get(i).getDescription());
            if(rel.getTickets().get(i).getTitle().equals("Fix Bug on Save")) {
                return rel.getTickets().get(i);
            }
            i++;
        }
       return null;
